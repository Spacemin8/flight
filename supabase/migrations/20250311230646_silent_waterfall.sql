...
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_flight_api_config_updated_at') THEN
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Admin can manage API config" ON flight_api_config;
  DROP POLICY IF EXISTS "Admin can manage API stats" ON flight_api_stats;
  DROP POLICY IF EXISTS "Public can read API stats" ON flight_api_stats;
EXCEPTION
