...
-- Add new columns to processed_flight_prices if they don't exist
DO $$ 
BEGIN
  -- Add airline column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'processed_flight_prices' AND column_name = 'airline'
  ) THEN
    ALTER TABLE processed_flight_prices ADD COLUMN airline text;
  END IF;

  -- Add flight_type column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'processed_flight_prices' AND column_name = 'flight_type'
  ) THEN
    ALTER TABLE processed_flight_prices ADD COLUMN flight_type text;
  END IF;

  -- Add stops column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'processed_flight_prices' AND column_name = 'stops'
  ) THEN
    ALTER TABLE processed_flight_prices ADD COLUMN stops integer;
  END IF;

  -- Add duration column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'processed_flight_prices' AND column_name = 'duration'
  ) THEN
    ALTER TABLE processed_flight_prices ADD COLUMN duration integer;
  END IF;
END $$;
