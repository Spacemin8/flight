...
BEGIN
  ...
  IF v_connection.from_type = 'city' THEN
    ...
    v_url := REPLACE(v_url, '{per_city}', COALESCE(v_connection.from_per, 'Për ' || v_connection.from_city));
  ...
  IF v_connection.to_type = 'city' THEN
    ...
    v_url := REPLACE(v_url, '{per_city}', COALESCE(v_connection.to_per, 'Për ' || v_connection.to_city));
  ...
  IF v_connection.from_type = 'state' THEN
    ...
    v_url := REPLACE(v_url, '{per_state}', COALESCE(v_connection.from_per, 'Për ' || v_connection.from_state));
  ...
  IF v_connection.to_type = 'state' THEN
    ...
    v_url := REPLACE(v_url, '{per_state}', COALESCE(v_connection.to_per, 'Për ' || v_connection.to_state));
...
COMMENT ON FUNCTION generate_route_template IS 
'Generates SEO template for a route connection based on location types and formats';

COMMENT ON FUNCTION handle_connection_status_change IS 
'Handles template generation when route connection status changes';
