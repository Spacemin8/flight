-- Add template_url column to seo_location_connections
ALTER TABLE seo_location_connections
ADD COLUMN template_url text;

-- Create index for template_url
CREATE INDEX idx_location_connections_template_url ON seo_location_connections(template_url);

-- Add helpful comment
COMMENT ON COLUMN seo_location_connections.template_url IS 
'The URL path where this route''s SEO page can be accessed';

-- Update existing connections with template URLs
DO $$
DECLARE
  v_connection record;
  v_url text;
