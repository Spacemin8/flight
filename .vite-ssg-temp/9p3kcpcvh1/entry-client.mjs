...
const supabase$1 = createClient(supabaseUrl$1, supabaseAnonKey$1, {
  auth: {
    persistSession: isBrowser$2,
    autoRefreshToken: isBrowser$2,
    detectSessionInUrl: isBrowser$2,
    storage: customStorage$1
  },
  realtime: {
    params: {
      eventsPerSecond: 2
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (shouldRetry && retries < maxRetries) {
        ...
        if (isBrowser$2 && (error.message.includes("session_not_found") || error.message.includes("JWT expired"))) {
          try {
            const {
              data: { session },
              error: refreshError
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        const {
          data: { subscription }
        } = supabase$1.auth.onAuthStateChange(async (event, session2) => {
          if (event === "SIGNED_OUT" || event === "USER_DELETED") {
            setUser(null);
            localStorage.removeItem("supabase.auth.token");
            sessionStorage.removeItem("supabase.auth.token");
